...
VERSIÓN FINAL - Importa desde apis_agent/agent.py
"""
...
try:
    # Intento 1: Desde src.agents.apis_agent.agent
    ...
    try:
        # Intento 2: Desde agents.apis_agent.agent
        ...
        logger.warning(f"⚠️  APIsAgent import failed: {str(e2)}")
...
try:
    ...
    logger.error(f"❌ APIsAgent error: {str(e)}")
    _apis_agent = None
...
async def generate_endpoints(
    ...
    try:
        ...
        logger.info(f"✅ Generated {len(endpoints_summary)} endpoints for {table_name}")
