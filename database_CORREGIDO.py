...
class ColumnDefinition:
    def __init__(self, name: str, type: str, primary_key: bool = False, nullable: bool = True):
        self.name = name
        self.type = type
...
class TableDefinition:
    def __init__(self, name: str, columns: List[Dict[str, Any]], description: str = ""):
        self.name = name
        self.columns = [ColumnDefinition(**col) if isinstance(col, dict) else col for col in columns]
...
async def create_table(
    ...
    try:
        ...
        # Validar que empresa_id y bodega_id existen (multisector)
        column_names = [col.get("name") for col in columns]
        if "empresa_id" not in column_names or "bodega_id" not in column_names:
            logger.warning(f"Table {table_name} missing enterprise columns (empresa_id, bodega_id)")
...
async def apply_migration(
    ...
    try:
        ...
        if not any(keyword in migration_script.upper() for keyword in ["ALTER", "ADD", "DROP", "MODIFY"]):
            raise HTTPException(
                status_code=400,
                detail="Migration script must contain ALTER, ADD, DROP, or MODIFY"
            )
...
def _generate_ddl(table_name: str, columns: List[Dict[str, Any]]) -> str:
    """Genera DDL básico para la tabla"""
