-- ============================================================================
-- ESQUEMA DE REGLAS DE NEGOCIO - SQL SERVER (EN ESPAÑOL)
...
CREATE TABLE reglas_negocio (
    ...
    -- ÍNDICES
    INDEX idx_alcance ON reglas_negocio(nivel_alcance, linea_negocio, id_empresa),
    INDEX idx_estado ON reglas_negocio(estado),
    INDEX idx_prioridad ON reglas_negocio(prioridad DESC)
);
...
CREATE TABLE condiciones_regla (
    ...
    INDEX idx_id_regla ON condiciones_regla(id_regla)
);
...
CREATE TABLE acciones_regla (
    ...
    FOREIGN KEY (id_regla) REFERENCES reglas_negocio(id_regla) ON DELETE CASCADE,
    INDEX idx_id_regla ON acciones_regla(id_regla)
);
...
CREATE TABLE auditoria_evaluacion_reglas (
    ...
    INDEX idx_id_regla ON auditoria_evaluacion_reglas(id_regla),
    INDEX idx_fecha_evaluacion ON auditoria_evaluacion_reglas(fecha_evaluacion),
    INDEX idx_id_empresa ON auditoria_evaluacion_reglas(id_empresa),
    INDEX idx_decision ON auditoria_evaluacion_reglas(decision)
);
...
CREATE TABLE cache_evaluacion_reglas (
    ...
    INDEX idx_regla_contexto ON cache_evaluacion_reglas(id_regla, hash_contexto)
);
...
CREATE TABLE herencia_reglas (
    ...
    INDEX idx_regla_padre ON herencia_reglas(id_regla_padre),
    INDEX idx_regla_hija ON herencia_reglas(id_regla_hija)
);
...
CREATE PROCEDURE sp_crear_regla_negocio
    ...
    @prioridad INT = 50,
    @creada_por NVARCHAR(100),
    @json_condiciones NVARCHAR(MAX),  -- JSON array
    @json_acciones NVARCHAR(MAX)      -- JSON array
AS
BEGIN
    BEGIN TRY
        BEGIN TRANSACTION;
...
CREATE PROCEDURE sp_evaluar_regla_jerarquica
    ...
    @id_empresa INT,
    @linea_negocio NVARCHAR(50),
    @json_contexto NVARCHAR(MAX),   -- {"id_cliente": 523, "monto": 10000, ...}
    @evaluada_por NVARCHAR(100) = 'sistema'
AS
BEGIN
    DECLARE @id_auditoria BIGINT;
    DECLARE @condiciones_cumplidas BIT = 1;
    DECLARE @resultado_json NVARCHAR(MAX);
    DECLARE @valores_calculados NVARCHAR(MAX) = '{}';
    DECLARE @decision NVARCHAR(MAX) = 'permitida';
    DECLARE @fecha_inicio DATETIME = GETUTCDATE();
    
    BEGIN TRY
        -- 1. EVALUAR NIVEL 1: GLOBAL
        DECLARE @reglas_globales NVARCHAR(MAX) = (
            SELECT STRING_AGG(id_regla, ',')
            FROM reglas_negocio
            WHERE nivel_alcance = 'global' AND estado = 'activa'
        );
        
        -- 2. EVALUAR NIVEL 2: LÍNEA DE NEGOCIO
        DECLARE @reglas_linea NVARCHAR(MAX) = (
            SELECT STRING_AGG(id_regla, ',')
            FROM reglas_negocio
            WHERE nivel_alcance = 'linea_negocio' 
                AND linea_negocio = @linea_negocio 
                AND estado = 'activa'
        );
        
        -- 3. EVALUAR NIVEL 3: EMPRESA
        DECLARE @reglas_empresa NVARCHAR(MAX) = (
            SELECT STRING_AGG(id_regla, ',')
            FROM reglas_negocio
            WHERE nivel_alcance = 'empresa' 
                AND id_empresa = @id_empresa 
                AND estado = 'activa'
        );
        
        -- 4. EVALUAR CONDICIONES DE LA REGLA ESPECÍFICA
        ...
        -- 5. EVALUAR CADA CONDICIÓN
        DECLARE @campo NVARCHAR(100), @operador NVARCHAR(20), @valor NVARCHAR(MAX);
        ...
        -- 6. SI CONDICIONES SE CUMPLEN, EJECUTAR ACCIONES
        ...
        BEGIN
            ...
            SELECT tipo_accion, detalles FROM acciones_regla WHERE id_regla = @id_regla ORDER BY orden_ejecucion;
            ...
            BEGIN
                ...
                BEGIN
                    DECLARE @nombre_var NVARCHAR(50) = JSON_VALUE(@detalles_accion, '$.variable');
                    DECLARE @formula NVARCHAR(MAX) = JSON_VALUE(@detalles_accion, '$.formula');
                    DECLARE @monto DECIMAL(18,2) = JSON_VALUE(@json_contexto, '$.monto');
                    DECLARE @resultado_calc DECIMAL(18,2);
                    
                    -- Ejecutar fórmula simple (retención = monto * 0.025)
                    IF @formula LIKE '%* 0.025%'
                        SET @resultado_calc = @monto * 0.025;
                    ELSE IF @formula LIKE '%* 0.03%'
                        SET @resultado_calc = @monto * 0.03;
                ...
                BEGIN
                    ...
                    SET @decision = JSON_MODIFY(@decision, '$.razon', JSON_VALUE(@detalles_accion, '$.razon'));
                END
        ...
        VALUES (
            @id_regla,
            (SELECT nivel_alcance FROM reglas_negocio WHERE id_regla = @id_regla),
            @linea_negocio,
            @id_empresa,
            @json_contexto,
...
CREATE PROCEDURE sp_listar_reglas_por_alcance
    @nivel_alcance NVARCHAR(20) = NULL,
    @linea_negocio NVARCHAR(50) = NULL,
    @id_empresa INT = NULL,
...
BEGIN
    ...
    FROM reglas_negocio
    WHERE
        (@nivel_alcance IS NULL OR nivel_alcance = @nivel_alcance)
        AND (@linea_negocio IS NULL OR linea_negocio = @linea_negocio)
...
BEGIN
    ...
    WHERE
        (@id_regla IS NULL OR id_regla = @id_regla)
        AND (@id_empresa IS NULL OR id_empresa = @id_empresa)
        AND fecha_evaluacion >= DATEADD(DAY, -@dias, GETUTCDATE())
...
BEGIN
    BEGIN TRY
        ...
        BEGIN
            DELETE FROM condiciones_regla WHERE id_regla = @id_regla;
        ...
        BEGIN
            DELETE FROM acciones_regla WHERE id_regla = @id_regla;
...
-- Vista: Reglas Activas por Nivel
